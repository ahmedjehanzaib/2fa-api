...
export function practicesRouter(): Router {
	...
	router.post('/', async (req: Request, res: Response, _next: NextFunction) => {
		try {
			...
			if (validated.error === null) {
                const client = await practicesFacade.createAPractice({
                    id: uuidv4(),
					...req.body
                })
	...
	router.get('/:id',  async (req: Request, res: Response, _next: NextFunction) => {
		try {
			...
			if (validated.error === null) {
				const client = await practicesFacade.findPracticeById(req.params.id);
				if (!client.length) {
		...
		try {
			...
			if (validated.error === null) {
				...
				if (!client.length) {
					...
					const deletedClient = await practicesFacade.deletePracticeById(req.params.id);
